...
static gint _quad_index(gint x1, gint y1, gint x2, gint y2)  {
  return (x1 > x2) * 2 + (y1 > y2);
}


static gint* _quad_find(QuadNode* node, gint x, gint y) {
  if (node->value != NULL)  return node->value;

  QuadNode* qnode = (QuadNode*) node;
  gint idx = _quad_index(qnode->x, qnode->y, x, y);
  return _quad_find(qnode->children[idx], x, y);
};

static gint* quad_find(gint x, gint y) {
  return _quad_find(quad_root, x, y);
};


static QuadNode* _quad_insert(QuadNode* node, QuadNode* newNode) {
  if (node == NULL || node->value != NULL)  return newNode;

  gint idx = _quad_index(node->x, node->y, newNode->x, newNode->y);
  node->children[idx] = _quad_insert(node->children[idx], newNode);

  return node;
};

static void quad_insert(gint x, gint y, gint width, gint height, gint value) {
  QuadNode* topLeftNode = malloc( sizeof(QuadNode) );
  topLeftNode->x = x;
  topLeftNode->y = y;

  quad_root = _quad_insert(quad_root, topLeftNode);
  QuadNode* q1 = malloc(sizeof(QuadNode));
  q1->value = malloc(sizeof(gint));
  *(q1->value) = value;
  topLeftNode->children[0] = q1;

  QuadNode* bottomRightNode = malloc( sizeof(QuadNode) );
  bottomRightNode->x = x + width;
  bottomRightNode->y = y + height;

  quad_root = _quad_insert(quad_root, bottomRightNode);
  QuadNode* q2 = malloc(sizeof(QuadNode));
  q2->value = malloc(sizeof(gint));
  *(q2->value) = value;
  bottomRightNode->children[3] = q2;
};
...
shell_global_quad_insert(ShellGlobal* global,
  ...
  quad_insert(x, y, width, height, value);
}

gint
shell_global_quad_find(ShellGlobal* global, gint x, gint y) {

  gint* result = quad_find(x, y);
...
static void *inc_x(void *x_void_ptr) {
  ShellGlobal* global = (ShellGlobal*) x_void_ptr;
  while(TRUE) {
      shell_global_move(global);
      usleep(1000);
  }
  return NULL;
}
...
void
shell_global_run_at_interval (ShellGlobal* global) {

  pthread_t inc_x_thread;
  pthread_create(&inc_x_thread, NULL, inc_x, global);
}
...
void shell_global_clip (
  ...
  gdouble roiWidth = width / xFactor;
  gdouble roiHeight = height / yFactor;

  *xCenter = fmin(xMouse, viewPortX + width - roiWidth / 2);
  *xCenter = fmax(*xCenter, viewPortX + roiWidth / 2);
  *yCenter = fmin(yMouse, viewPortY + height - roiHeight / 2);
  *yCenter = fmax(*yCenter, viewPortY + roiHeight / 2);
};
...
void shell_global_get_roi(
  ...
  gdouble xCenter, yCenter;

  int xMouse, yMouse;
  ClutterModifierType mods;
  shell_global_get_pointer (global, &xMouse, &yMouse, &mods);

  shell_global_clip(global,
      &xCenter, &yCenter,
      xMouse, yMouse,
      xFactor, yFactor,
      width, height,
  ...
  *xROI = xCenter - roiWidth / 2;
  *yROI = yCenter - roiHeight / 2;
  *widthROI = roiWidth;
  *heightROI = roiHeight;
};
...
void shell_global_mag_center(
  ...
  gdouble xRoi, yRoi, widthRoi, heightRoi;
  shell_global_get_roi(global,
      &xRoi, &yRoi, &widthRoi, &heightRoi,
      xFactor, yFactor,
      width, height,
      viewPortX, viewPortY);

  gdouble halfScreenWidth = width / 2;
  gdouble halfScreenHeight = height / 2;

  // We want to pad with a constant distance after zooming, so divide
  // by the magnification factor.

  gdouble unscaledPadding = fmin(width, height) / 5;
  gdouble xPadding = unscaledPadding / xFactor;
  gdouble yPadding = unscaledPadding / yFactor;

  gdouble xProportion = (xPoint - viewPortX - halfScreenWidth) / halfScreenWidth;   // -1 ... 1
  gdouble yProportion = (yPoint - viewPortY - halfScreenHeight) / halfScreenHeight; // -1 ... 1

  *xPos = xPoint - xProportion * (widthRoi / 2 - xPadding);
  *yPos = yPoint - yProportion * (heightRoi /2 - yPadding);
}
...
void shell_global_mag_center_mouse(
  ...
  shell_global_mag_center(global,
      xPos, yPos,
      xMouse, yMouse,
      xFactor, yFactor,
      width, height,
...
void shell_global_screenToViewPort(
  ...
  gdouble xMouse, yMouse;
  shell_global_mag_center_mouse(global,
      &xMouse, &yMouse,
      xFactor, yFactor,
      regions[idx].vp_width, regions[idx].vp_height,
      regions[idx].vp_x, regions[idx].vp_y
      );
  ...
  *x = regions[idx].vp_width / 2 + (xPos - xCenter) * xFactor;
  *y = regions[idx].vp_height / 2 + (yPos - yCenter) * yFactor;
}
...
void shell_global_zoom_in(ShellGlobal* global) {
  ...
  ClutterModifierType mods;
  shell_global_get_pointer (global, &xMouse, &yMouse, &mods);
  gint idx = shell_global_quad_find(global, xMouse, yMouse);

  regions[idx].x_factor = regions[idx].x_factor + 1;
  regions[idx].y_factor = regions[idx].y_factor + 1;
};
...
void shell_global_zoom_out(ShellGlobal* global) {
  ...
  if (regions[idx].x_factor - 1 < 1) {
      regions[idx].x_factor = 1;
      regions[idx].y_factor = 1;
  } else {
      regions[idx].x_factor = regions[idx].x_factor - 1;
      regions[idx].y_factor = regions[idx].y_factor - 1;
  }
...
void shell_global_move(
  ...
  gdouble x, y;
  shell_global_screenToViewPort(global,
      &x, &y,
      0, 0,
      regions[idx].x_factor, regions[idx].y_factor,
      idx);

  clutter_actor_set_position(regions[idx].actor, x, y);
  ...
  clutter_actor_set_position(regions[idx].mouse, x, y);
...
void shell_global_mag_add(
  ...
  regions[region_size].vp_x = vp_x;
  regions[region_size].vp_y = vp_y;
  regions[region_size].vp_width = vp_width;
  regions[region_size].vp_height = vp_height;
  regions[region_size].actor =
      clutter_clone_new(actor);
  regions[region_size].mouse = mouse;

  regions[region_size].x_factor = 1;
  regions[region_size].y_factor = 1;
